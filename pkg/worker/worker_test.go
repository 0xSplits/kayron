...
// Test_Worker_handler_execution verifies the isolation of handler failure
// domains so that the failure of one handler does not affect the execution of
...
func Test_Worker_handler_execution(t *testing.T) {
	...
	// register. Once the unbuffered ready channel is closed, <-wrk.rdy unblocks
